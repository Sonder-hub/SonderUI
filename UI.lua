...
if game.CoreGui:FindFirstChild('OwennUiLib') then
    game.CoreGui:FindFirstChild('OwennUiLib'):Destroy()
    destroyed = true
end
...
local OwennUiLib = Instance.new("ScreenGui")

OwennUiLib.Name = "OwennUiLib"

protect_gui(OwennUiLib)
...
function library:Keybind(key)
    ...
    keybindConnection = uis.InputBegan:Connect(function(input, gp)
        if not gp and input.KeyCode == Enum.KeyCode[key] then
            OwennUiLib.Enabled = not OwennUiLib.Enabled
        end
    end)
end
...
function library:Window(name) 
    ...
    Header.BackgroundColor3 = Color3.fromRGB(31, 121, 239)
